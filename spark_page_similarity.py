...
class Pages:
	def __init__(self):
		...
		self.table = self.hbase.table("spark_pages_similarity")

	def create_similarity_hbase_table(self):
		self.hbase_connection = self.hbase.get_connection()
		try:
			self.hbase_connection.create_table("spark_pages_similarity", {'paired_page':dict()})
...
def spark_similarity(data, stop_words):
	spark_pages = Pages()
	conf = SparkConf().setAppName('J_Similarity').setMaster('local[4]')
